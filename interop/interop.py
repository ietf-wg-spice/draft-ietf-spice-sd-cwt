...
def key2alg(cosekey):
    ...
    crv2alg = {
        ...
        7: -8   # Ed448   ==> EdDSA
        #
        #  ML-DSA isn't a *curve* it uses "kty" (1) = "AKP" (7)
        #  the algorithm numbers are as follows:
        #    -48    # ML-DSA-44
        #    -49    # ML-DSA-65
        #    -50    # ML-DSA-87
    }
...
def testname(issuer, holder, verifier, timestamp=None):
    suffix = None
    if timestamp:
        suffix = now()
    if holder is None:
        raise Exception("No Holder implementation")
    if issuer is None:
        if verifier is None:
            raise Exception("Must have either an Issuer or a Verifier or both")
        else:
            return f"H={holder}_V={verifier}" + suffix
    else:
        if verfier is None:
            return f"I={issuer}_H={holder}" + suffix
        else:
            return f"I={issuer}_H={holder}_V={verifier}" + suffix
