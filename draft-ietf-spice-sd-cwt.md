...
When the Verifier requires freshness, it provides a nonce that the Holder signs in the SD-KBT as the `cnonce` claim ({{sd-kbt-claims}}).
...
The protected header of the SD-KBT MUST include the `typ` header parameter with the unsigned integer value of 294, or the value `application/kb+cwt`.
...
The Holder finally signs the SD-KBT containing all intended disclosures of either type.
