...
This specification includes two options for presented encrypted disclosures.
The first is useful when an appropriate symmetric key and its context are provided completely out-of-band.
For example, the MLS protocol can produce a secret that is only known to the members of a specific group in a specific MLS epoch.
In this first case, the Holder encrypts the encrypted disclosures directly using an IANA registered Authenticated Encryption with Additional Data (AEAD) algorithm.

In the second case, the context is conveyed in-band using COSE.

In both cases, the entire SD-CWT is included in the protected header of the SD-KBT, which secures the entire Issuer-signed SD-CWT including its unprotected headers that include its disclosures.
